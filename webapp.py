...
# ── Auth config ──
AUTH_EMAIL = os.environ.get("AUTH_EMAIL", "admin")
_raw_password = os.environ.get("AUTH_PASSWORD", "admin")
...
def api_sheets_auto_fill():
    try:
        ...
        if not google_sheets.is_configured():
            return jsonify({"ok": False, "error": "Google Sheets not configured (no service account). This feature is optional and not needed for booking."})
